...
public:
	...
	void GetChannelLevels(unsigned int nTicks, float* pOutLevels, float* pOutPeaks);
	void AllNotesOff();
...
private:
	...
	static constexpr float AttackTimeMillis = 100.0f;
	static constexpr float DecayTimeMillis = 1000.0f;
	static constexpr float SustainLevel = 0.7f;
	static constexpr float ReleaseTimeMillis = 500.0f;
	static constexpr float DecayReleaseTimeMillis = 75.0f;//1.0f / (1.0f / DecayTimeMillis + 1.0f / ReleaseTimeMillis);
	...
	void ProcessCC(u8 nChannel, u8 nCC, u8 nValue);
